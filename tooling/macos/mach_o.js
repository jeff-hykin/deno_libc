import { Buffer } from "../buffer_shim.js"
...
export class Parser extends Reader {
    ...
    parseHead(buf) {
        ...
        var bits
        if (magic === 0xfeedface || magic === 0xcefaedfe) bits = 32
        else if (magic === 0xfeedfacf || magic == 0xcffaedfe) bits = 64
        else return false

        if (magic & (0xff == 0xfe)) this.setEndian("be")
        else this.setEndian("le")
        ...
        // Get endian
        var endian
        if ((cpusubtype & constants.endian.multiple) === constants.endian.multiple) endian = "multiple"
        else if (cpusubtype & constants.endian.be) endian = "be"
        else endian = "le"
        ...
        // Get subtype
        var subtype
        if (endian === "multiple") subtype = "all"
        else if (cpusubtype === 0) subtype = "none"
        else subtype = constants.cpuSubType[cputype][cpusubtype]
    ...
    parseCStr(buf) {
        for (var i = 0; i < buf.length; i++) if (buf[i] === 0) break
    ...
    parseFunctionStarts(type, buf, file) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if ((data[i] & 0x80) !== 0) {
                ...
                shift += 7
                if (shift > 24) throw new Error("function_starts delta too large")
                else if (i + 1 === data.length) throw new Error("function_starts delta truncated")
            } else if (delta === 0) {
                // end of table
                break
            } else {
